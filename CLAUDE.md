...
- The `INSTRUCT` array in each `lesson-N-specific.js` drives the entire instruction phase
- Each `INSTRUCT` step has: `build()`, `audio()`, `question`, `acc[]`, `cor`, `fu()`, `fuQ`, `fuAcc[]`, `fuCor`
- `acc[]` first item is always the correct button option — other items generate distractors
- Bar models are never edited directly — they are generated by functions like `genPW()`, `genFrac()`
- `CHAR_DELAY_MS` in `lesson-template.html` controls text animation speed for all lessons
- `patch.js` can find-and-replace across all files — set `FIND` and `REPLACE` then run `node patch.js`
